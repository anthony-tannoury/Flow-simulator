...
EDITOR_VERSION = "0.2.0-new-simulator"
...
def get_int_prop_with_aliases(node: BaseNode, keys: tuple[str, ...], default: int) -> int:
    """Read the first available integer property from a node, keeping older clean JSON compatible."""
    for key in keys:
        if node.has_property(key):
            return as_int(node.get_property(key), default)
    return default


def get_bool_prop_with_aliases(node: BaseNode, keys: tuple[str, ...], default: bool) -> bool:
    """Read the first available boolean property from a node, keeping older clean JSON compatible."""
    for key in keys:
        if node.has_property(key):
...
def normalize_batch_collector(value: Any) -> str:
    """Normalize older UI names to the latest simulator CollectorType enum names."""
    mapping = {
        ...
        "DISCRIMINATING_GREEDY": "DISCRIMINATING_GREEDY",
        "NON_DISCRIMINATING_GREEDY": "NON_DISCRIMINATING_GREEDY",
        "DISCRIMINATING_ALTRUISTIC": "DISCRIMINATING_ALTRUISTIC",
        "NON_DISCRIMINATING_ALTRUISTIC": "NON_DISCRIMINATING_ALTRUISTIC",
    }
    return mapping.get(str(value), "DISCRIMINATING_GREEDY")


def normalize_groups_from_flat(items: list[dict], quantity_type=float) -> list[list[dict]]:
    """Convert legacy flat [{resource, quantity}, ...] into the new list-of-alternative-groups shape.

    In the latest simulator, resources/operators are expressed as a list of alternative groups:
        [[A, B], [C]] means (A AND B) OR C.
    For backward compatibility, a flat old list means one group containing all entries.
    """
    group = []
    for item in items or []:
        ...
        if rid:
            ...
            q = int(q) if quantity_type is int else float(q)
            group.append({"resource": rid, "quantity": q})
    return [group] if group else []


def groups_from_property_or_connections(node: BaseNode, prop_name: str, port_name: str, integer: bool) -> list[list[dict]]:
    groups = get_property_json(node, prop_name, [])
    if groups:
        return groups
    refs = connected_refs_from_port(node, port_name, "input")
    if not refs:
        return []
    q = 1 if integer else 1.0
    # Default behavior: all connected resources are required together, matching the old simulator semantics.
    return [[{"resource": rid, "quantity": q} for rid in refs]]


def flatten_resource_items(raw) -> list[dict]:
    """Accept either latest [[{...}], [{...}]] groups or legacy [{...}] flat lists."""
    if not raw:
        return []
...
class NonFlexibleShutdownsNode(SimNode):
    NODE_NAME = "Non-Flexible Shutdowns"
    kind = "NonFlexibleShutdowns"
    color = (125, 80, 130)

    def __init__(self):
        super().__init__()
        self.add_input("intervals", multi_input=True, color=PORT_COLORS["interval"])
        self.add_output("non_flexible_shutdowns", color=PORT_COLORS["shutdown"])
...
class FlexibleShutdownsNode(SimNode):
    NODE_NAME = "Flexible Shutdowns"
    kind = "FlexibleShutdowns"
    color = (115, 90, 145)

    def __init__(self):
        super().__init__()
        self.add_input("intervals", multi_input=True, color=PORT_COLORS["interval"])
        self.add_output("flexible_shutdowns", color=PORT_COLORS["shutdown_flexible"])
...
class ScheduledShutdownsNode(NonFlexibleShutdownsNode):
    """Legacy node kept for old clean JSON imports. Prefer NonFlexibleShutdownsNode."""
    NODE_NAME = "Scheduled Shutdowns (legacy)"
    kind = "ScheduledShutdowns"

    def __init__(self):
        super().__init__()
        self.set_color(125, 80, 130)

    def to_clean_json(self) -> dict:
        data = super().to_clean_json()
        data["kind"] = "NonFlexibleShutdowns"
        data["legacy_kind"] = "ScheduledShutdowns"
        return data
...
class ResourceNode(SimNode):
    ...
    def __init__(self):
        ...
        # Fixed behavior: editor no longer allows making normal resources anonymous.
        # Keep the property only for backward compatibility with older JSON/code.
        self.create_property("anonymous", False)
...
class PieceGeneratorNode(SimNode):
    ...
    def __init__(self):
        ...
        self.create_property("models_goals", "[]")
        add_float_input(self, "working_hours", "working_hours", 480.0)

    def to_clean_json(self) -> dict:
        goals = []
        for item in get_property_json(self, "models_goals", []):
            model = item.get("model")
            goal = as_int(item.get("goal", item.get("quantity", 0)), 0)
            if model and goal > 0:
                goals.append({"model": model, "goal": goal})
        return {
            "id": node_uid(self),
            "kind": self.kind,
            "name": self.name(),
            "models_goals": goals,
            "working_hours": as_float(self.get_property("working_hours"), 480.0),
            "bufs_out": get_output_refs(self, "bufs_out"),
            "position": [self.x_pos(), self.y_pos()],
        }


class FirstTaskNode(PieceGeneratorNode):
    """Legacy name kept so older templates/imports still open. Export is now PieceGenerator."""
    NODE_NAME = "First Task (legacy generator)"
    kind = "FirstTask"

    def __init__(self):
        super().__init__()
        # Legacy properties imported from older files. They are converted in apply_clean_json_to_node.
        if not self.has_property("models_probs"):
            self.create_property("models_probs", "[]")
        if not self.has_property("resource_quantities"):
            self.create_property("resource_quantities", "{}")

    def to_clean_json(self) -> dict:
        data = super().to_clean_json()
        data["legacy_kind"] = "FirstTask"
        return data
...
class TaskNode(SimNode):
    ...
    def __init__(self):
        ...
        self.add_input("non_flexible_shutdowns", color=PORT_COLORS["shutdown"])
        self.add_input("scheduled_shutdowns", color=PORT_COLORS["shutdown"])  # legacy alias for non-flexible shutdowns
        self.add_input("flexible_shutdowns", color=PORT_COLORS["shutdown_flexible"])

        self.add_output("bufs_out", multi_output=True, color=PORT_COLORS["task"])
        self.add_output("task_ref", multi_output=True, color=PORT_COLORS["breakdown"])

        # Latest simulator PieceTaskConfig fields.
        self.create_property("capability", "[]")  # legacy/simple list of models; converted to models_configs on export.
        self.create_property("models_configs", "[]")  # advanced per-model configs: model, duration, resources groups.
        self.create_property("resources_scope", "PER_PIECE")
        self.create_property("operators_scope", "PER_BATCH")
        self.create_property("default_resource_groups", "[]")
        self.create_property("operator_groups", "[]")
        self.create_property("startup_operator_groups", "[]")
        # Legacy flat quantity maps still accepted.
        self.create_property("resource_quantities", "{}")
        ...
        self.create_property("min_carrier_capacity", 1)
        self.create_property("max_carrier_capacity", 1)
        self.create_property("contiguous_carriers", False)
        self.create_property("collector_type", "DISCRIMINATING_GREEDY")
        self.create_property("independent_carriers", False)

    def _default_model_resource_groups(self) -> list[list[dict]]:
        groups = get_property_json(self, "default_resource_groups", [])
        if groups:
            return groups
        # Legacy fallback: one AND-group containing all connected resources and their quantities.
        resources = connected_refs_from_port(self, "resources", "input")
        rq = get_property_json(self, "resource_quantities", {})
        group = [{"resource": rid, "quantity": as_float(rq.get(rid, 1.0), 1.0)} for rid in resources]
        return [group] if group else []

    def _models_configs(self) -> list[dict]:
        configs = get_property_json(self, "models_configs", [])
        if configs:
            return configs
        duration_ref = get_input_ref(self, "task_duration")
        resource_groups = self._default_model_resource_groups()
        result = []
        for model in get_property_json(self, "capability", []):
            result.append({
                "model": model,
    ...
    def to_clean_json(self) -> dict:
        return {
            "id": node_uid(self),
            "kind": "PieceTask",
            "editor_kind": self.kind,
            "name": self.name(),
            "models_configs": self._models_configs(),
            # Legacy key kept for older parsers/tools that still inspect capability.
            ...
            "operators": groups_from_property_or_connections(self, "operator_groups", "operators", integer=True),
            "operators_scope": self.get_property("operators_scope"),
            "startup_operators": groups_from_property_or_connections(self, "startup_operator_groups", "startup_operators", integer=True),
            "startup_duration": get_input_ref(self, "startup_duration"),
            "min_carriers": get_int_prop_with_aliases(self, ("min_carriers",), 1),
            ...
            "contiguous_carriers": get_bool_prop_with_aliases(self, ("contiguous_carriers", "greedy_carriers"), False),
            "collector_type": normalize_batch_collector(
                self.get_property("collector_type")
                if self.has_property("collector_type")
                else self.get_property("batch_collector_type")
            ),
            "independent_carriers": bool(self.get_property("independent_carriers")),
            "non_flexible_shutdowns": get_input_ref(self, "non_flexible_shutdowns") or get_input_ref(self, "scheduled_shutdowns"),
            "flexible_shutdowns": get_input_ref(self, "flexible_shutdowns"),
            # Backward compatible field name.
            "scheduled_shutdowns": get_input_ref(self, "non_flexible_shutdowns") or get_input_ref(self, "scheduled_shutdowns"),
            "position": [self.x_pos(), self.y_pos()],
        }
...
class PieceGeneratorGoalsDialog(QtWidgets.QDialog):
    def __init__(self, parent, all_models: List[dict], current: List[dict]):
        super().__init__(parent)
        self.setWindowTitle("PieceGenerator model goals")
        self.resize(520, 380)
        self.all_model_names = [m["name"] for m in all_models]

        self.table = QtWidgets.QTableWidget(0, 2)
        self.table.setHorizontalHeaderLabels(["model", "goal"])
        self.table.horizontalHeader().setStretchLastSection(True)
        ...
        layout = QtWidgets.QVBoxLayout(self)
        layout.addWidget(QtWidgets.QLabel("Pick leaf models and target production goals. The generator will produce exactly these totals over working_hours."))
        layout.addWidget(self.table)
        ...
        for item in current:
            self.add_row(item.get("model", ""), item.get("goal", item.get("quantity", 1)))
    ...
    def values(self) -> List[dict]:
        ...
        for r in range(self.table.rowCount()):
            ...
            goal = as_int(item.text() if item else 0, 0)
            if not model:
                continue
            if model in seen:
                raise ValueError(f"Duplicate model in PieceGenerator goals: {model}")
            if goal <= 0:
                raise ValueError("Goals must be positive integers.")
            seen.add(model)
            result.append({"model": model, "goal": goal})
        if not result:
            raise ValueError("At least one model goal is required.")
        return result
    ...
    def accept(self):
        try:
            ...
            qmessage(self, "Invalid model goals", str(e), QtWidgets.QMessageBox.Warning)
            return
        super().accept()


class ResourceGroupsDialog(QtWidgets.QDialog):
    """Edit list-of-alternative resource groups.

    Rows with the same group number are requested together (AND).
    Different group numbers are alternatives (OR).
    """
    def __init__(self, parent, title: str, connected_nodes: List[BaseNode], current_groups: list, integer=False):
        super().__init__(parent)
        self.setWindowTitle(title)
        self.resize(680, 420)
        self.connected_nodes = connected_nodes
        self.integer = integer
        self.uid_to_node = {node_uid(n): n for n in connected_nodes}

        self.table = QtWidgets.QTableWidget(0, 3)
        self.table.setHorizontalHeaderLabels(["alternative group", "resource", "quantity"])
        self.table.horizontalHeader().setStretchLastSection(True)

        btn_add = QtWidgets.QPushButton("Add row")
        btn_remove = QtWidgets.QPushButton("Remove selected")
        btn_default = QtWidgets.QPushButton("Default: all connected together")
        btn_ok = QtWidgets.QPushButton("OK")
        btn_cancel = QtWidgets.QPushButton("Cancel")
        ...
        layout = QtWidgets.QVBoxLayout(self)
        layout.addWidget(QtWidgets.QLabel("Same group number = requested together. Different group numbers = alternative choices."))
        layout.addWidget(self.table)
        layout.addLayout(tools)
        layout.addLayout(bottom)

        btn_add.clicked.connect(lambda: self.add_row(1, "", 1 if integer else 1.0))
        btn_remove.clicked.connect(self.remove_selected)
        btn_default.clicked.connect(self.fill_default)
        btn_ok.clicked.connect(self.accept)
        btn_cancel.clicked.connect(self.reject)

        if current_groups:
            for gi, group in enumerate(current_groups, start=1):
                for item in group:
                    self.add_row(gi, item.get("resource", ""), item.get("quantity", 1 if integer else 1.0))
        else:
            self.fill_default()

    def add_row(self, group=1, resource_id="", quantity=1):
        row = self.table.rowCount()
        self.table.insertRow(row)
        self.table.setItem(row, 0, QtWidgets.QTableWidgetItem(str(group)))
        combo = QtWidgets.QComboBox()
        for uid, node in self.uid_to_node.items():
            combo.addItem(f"{node.name()} [{uid}]", uid)
        if resource_id in self.uid_to_node:
            idx = combo.findData(resource_id)
            if idx >= 0:
                combo.setCurrentIndex(idx)
        self.table.setCellWidget(row, 1, combo)
        self.table.setItem(row, 2, QtWidgets.QTableWidgetItem(str(quantity)))

    def fill_default(self):
        self.table.setRowCount(0)
        for uid in self.uid_to_node:
            self.add_row(1, uid, 1 if self.integer else 1.0)
    ...
    def values(self) -> list[list[dict]]:
        groups = {}
        for r in range(self.table.rowCount()):
            group_item = self.table.item(r, 0)
            qty_item = self.table.item(r, 2)
            combo = self.table.cellWidget(r, 1)
            group_no = as_int(group_item.text() if group_item else 1, 1)
            rid = combo.currentData() if combo else None
            if not rid:
                continue
            if self.integer:
                q = as_int(qty_item.text() if qty_item else 1, 1)
                if q <= 0:
                    raise ValueError("Integer quantities must be positive.")
            else:
                q = as_float(qty_item.text() if qty_item else 1.0, 1.0)
                if q <= 0:
                    raise ValueError("Quantities must be positive.")
            groups.setdefault(group_no, []).append({"resource": rid, "quantity": q})
    ...
    def accept(self):
        try:
            ...
            qmessage(self, "Invalid resource groups", str(e), QtWidgets.QMessageBox.Warning)
            return
        super().accept()


class AdvancedModelConfigsDialog(QtWidgets.QDialog):
    """Advanced per-model PieceTaskConfig editor.

    This is intentionally JSON-based for resource groups so the designer can represent the latest simulator shape
    without forcing a large custom graph syntax. Leave empty to use the simple capability + common task_duration setup.
    """
    def __init__(self, parent, all_models: List[dict], current: List[dict]):
        super().__init__(parent)
        self.setWindowTitle("Advanced model configs")
        self.resize(900, 450)
        self.all_model_names = [m["name"] for m in all_models]
        self.table = QtWidgets.QTableWidget(0, 4)
        self.table.setHorizontalHeaderLabels(["model", "duration", "resources groups JSON", "note"])
        self.table.horizontalHeader().setStretchLastSection(True)
        ...
        bottom.addWidget(btn_cancel)
        layout = QtWidgets.QVBoxLayout(self)
        layout.addWidget(QtWidgets.QLabel('Duration can be a distribution node id or an embedded JSON like {"type":"Constant","params":{"value":5}}. Resources groups JSON uses [[{"resource":"id","quantity":1.0}]].'))
        layout.addWidget(self.table)
        layout.addLayout(tools)
        layout.addLayout(bottom)
        ...
        btn_cancel.clicked.connect(self.reject)
        for item in current:
            self.add_row(item.get("model", ""), item.get("duration", ""), json.dumps(item.get("resources", []), ensure_ascii=False), item.get("note", ""))

    def add_row(self, model="", duration="", resources="[]", note=""):
        row = self.table.rowCount()
        self.table.insertRow(row)
        combo = QtWidgets.QComboBox()
        combo.addItems(self.all_model_names)
        if model in self.all_model_names:
            combo.setCurrentText(model)
        self.table.setCellWidget(row, 0, combo)
        self.table.setItem(row, 1, QtWidgets.QTableWidgetItem(str(duration)))
        self.table.setItem(row, 2, QtWidgets.QTableWidgetItem(str(resources)))
        self.table.setItem(row, 3, QtWidgets.QTableWidgetItem(str(note)))
    ...
    def values(self) -> list[dict]:
        ...
        for r in range(self.table.rowCount()):
            combo = self.table.cellWidget(r, 0)
            model = combo.currentText().strip() if combo else ""
            duration_txt = self.table.item(r, 1).text().strip() if self.table.item(r, 1) else ""
            resources_txt = self.table.item(r, 2).text().strip() if self.table.item(r, 2) else "[]"
            note = self.table.item(r, 3).text().strip() if self.table.item(r, 3) else ""
            if not model:
                continue
            if model in seen:
                raise ValueError(f"Duplicate model config: {model}")
            seen.add(model)
            try:
                duration = json.loads(duration_txt) if duration_txt.startswith("{") else duration_txt
            except Exception as e:
                raise ValueError(f"Invalid duration JSON for {model}: {e}")
            try:
                resources = json.loads(resources_txt or "[]")
            except Exception as e:
                raise ValueError(f"Invalid resources JSON for {model}: {e}")
            result.append({"model": model, "duration": duration, "resources": resources, "note": note})
        return result
...
class TaskConfigDialog(QtWidgets.QDialog):
    def __init__(self, parent, task_node: TaskNode, model_registry: List[dict]):
        super().__init__(parent)
        self.setWindowTitle("PieceTask config")
        self.resize(620, 620)
        self.task_node = task_node
        self.model_registry = model_registry

        self.capability_btn = QtWidgets.QPushButton("Edit simple capability models...")
        self.capability_label = QtWidgets.QLabel(", ".join(get_property_json(task_node, "capability", [])) or "<none>")
        self.advanced_models_btn = QtWidgets.QPushButton("Advanced per-model configs...")
        ...
        self.min_carriers = QtWidgets.QSpinBox()
        self.min_carriers.setMinimum(1)
        self.min_carriers.setMaximum(999999)
        self.min_carriers.setValue(get_int_prop_with_aliases(task_node, ("min_carriers",), 1))

        self.max_capacity = QtWidgets.QDoubleSpinBox()
        self.max_capacity.setMinimum(0.000001)
        self.max_capacity.setMaximum(999999999)
        self.max_capacity.setDecimals(6)
        self.max_capacity.setValue(as_float(task_node.get_property("max_capacity"), 1.0))

        self.min_carrier_capacity = QtWidgets.QDoubleSpinBox()
        self.min_carrier_capacity.setMinimum(0.000001)
        self.min_carrier_capacity.setMaximum(999999999)
        self.min_carrier_capacity.setDecimals(6)
        self.min_carrier_capacity.setValue(as_float(task_node.get_property("min_carrier_capacity"), 1.0))

        self.max_carrier_capacity = QtWidgets.QDoubleSpinBox()
        self.max_carrier_capacity.setMinimum(0.000001)
        self.max_carrier_capacity.setMaximum(999999999)
        self.max_carrier_capacity.setDecimals(6)
        self.max_carrier_capacity.setValue(as_float(task_node.get_property("max_carrier_capacity"), 1.0))

        self.contiguous_carriers = QtWidgets.QCheckBox("contiguous_carriers")
        self.contiguous_carriers.setChecked(get_bool_prop_with_aliases(task_node, ("contiguous_carriers", "greedy_carriers"), False))

        self.collector_type = QtWidgets.QComboBox()
        self.collector_type.addItems(BATCH_COLLECTORS)
        current_collector = task_node.get_property("collector_type") if task_node.has_property("collector_type") else task_node.get_property("batch_collector_type")
        self.collector_type.setCurrentText(normalize_batch_collector(current_collector))
        ...
        btn_resources = QtWidgets.QPushButton("Edit default model resource groups...")
        btn_operators = QtWidgets.QPushButton("Edit operator alternative groups...")
        btn_startup_ops = QtWidgets.QPushButton("Edit startup operator alternative groups...")

        ok = QtWidgets.QPushButton("OK")
        cancel = QtWidgets.QPushButton("Cancel")

        form = QtWidgets.QFormLayout()
        form.addRow("simple capability", self.capability_label)
        form.addRow("", self.capability_btn)
        form.addRow("", self.advanced_models_btn)
        ...
        form.addRow("default model resources", btn_resources)
        ...
        self.advanced_models_btn.clicked.connect(self.edit_advanced_model_configs)
        btn_resources.clicked.connect(self.edit_default_resource_groups)
        btn_operators.clicked.connect(self.edit_operator_groups)
        btn_startup_ops.clicked.connect(self.edit_startup_operator_groups)
        ok.clicked.connect(self.accept)
        cancel.clicked.connect(self.reject)
    ...
    def edit_advanced_model_configs(self):
        current = get_property_json(self.task_node, "models_configs", [])
        dlg = AdvancedModelConfigsDialog(self, self.model_registry, current)
        if dlg.exec():
            set_property_json(self.task_node, "models_configs", dlg.values())

    def edit_default_resource_groups(self):
        connected = connected_nodes_from_port(self.task_node, "resources", "input")
        current = get_property_json(self.task_node, "default_resource_groups", [])
        if not current:
            current = normalize_groups_from_flat([
                {"resource": rid, "quantity": q}
                for rid, q in get_property_json(self.task_node, "resource_quantities", {}).items()
            ], quantity_type=float)
        dlg = ResourceGroupsDialog(self, "Default model resource groups", connected, current, integer=False)
        if dlg.exec():
            set_property_json(self.task_node, "default_resource_groups", dlg.values())

    def edit_operator_groups(self):
        connected = connected_nodes_from_port(self.task_node, "operators", "input")
        current = get_property_json(self.task_node, "operator_groups", [])
        if not current:
            current = normalize_groups_from_flat([
                {"resource": rid, "quantity": q}
                for rid, q in get_property_json(self.task_node, "operator_quantities", {}).items()
            ], quantity_type=int)
        dlg = ResourceGroupsDialog(self, "Operator alternative groups", connected, current, integer=True)
        if dlg.exec():
            set_property_json(self.task_node, "operator_groups", dlg.values())

    def edit_startup_operator_groups(self):
        connected = connected_nodes_from_port(self.task_node, "startup_operators", "input")
        current = get_property_json(self.task_node, "startup_operator_groups", [])
        if not current:
            current = normalize_groups_from_flat([
                {"resource": rid, "quantity": q}
                for rid, q in get_property_json(self.task_node, "startup_operator_quantities", {}).items()
            ], quantity_type=int)
        dlg = ResourceGroupsDialog(self, "Startup operator alternative groups", connected, current, integer=True)
        if dlg.exec():
            set_property_json(self.task_node, "startup_operator_groups", dlg.values())

    def accept(self):
        if self.min_carrier_capacity.value() > self.max_carrier_capacity.value():
            qmessage(self, "Invalid capacity", "min_carrier_capacity cannot be greater than max_carrier_capacity.", QtWidgets.QMessageBox.Warning)
            return
        if self.max_carrier_capacity.value() > self.max_capacity.value():
            qmessage(self, "Invalid capacity", "max_carrier_capacity cannot be greater than max_capacity.", QtWidgets.QMessageBox.Warning)
            return
        if self.min_carriers.value() * self.min_carrier_capacity.value() > self.max_capacity.value():
            qmessage(self, "Invalid capacity", "min_carriers × min_carrier_capacity cannot be greater than max_capacity.", QtWidgets.QMessageBox.Warning)
            return
...
def is_valid_connection(out_kind: str, out_port: str, in_kind: str, in_port: str) -> bool:
    """
    Strict connection rules for the latest simulator-oriented graph.
    Node kind "Task" represents a PieceTask in the exported clean JSON.
    """
    ...
    if out_kind == "Interval":
        return in_kind in {"NonFlexibleShutdowns", "FlexibleShutdowns", "ScheduledShutdowns"} and in_port == "intervals"

    if out_kind == "NonFlexibleShutdowns":
        return in_kind == "Task" and in_port == "non_flexible_shutdowns"

    if out_kind == "FlexibleShutdowns":
        return in_kind == "Task" and in_port == "flexible_shutdowns"

    # Legacy scheduled shutdowns behave as non-flexible shutdowns.
    if out_kind == "ScheduledShutdowns":
        return in_kind == "Task" and in_port in {"non_flexible_shutdowns", "scheduled_shutdowns"}

    if out_kind in {"Resource", "RestockableResource"}:
        return in_kind == "Task" and in_port in {"resources", "operators", "startup_operators"}
    ...
    if out_kind in {"Task", "PieceGenerator", "FirstTask", "Breakdown"} and out_port == "bufs_out":
...
class FlowEditorWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.graph.register_nodes([
            DistributionNode,
            IntervalNode,
            NonFlexibleShutdownsNode,
            FlexibleShutdownsNode,
            ScheduledShutdownsNode,  # legacy
            ResourceNode,
            RestockableResourceNode,
            HardBufferNode,
            SoftBufferNode,
            PieceGeneratorNode,
            FirstTaskNode,  # legacy
            TaskNode,
            BreakdownNode,
            MonitorNode,
        ])
    ...
    def _build_menus(self):
        ...
        add_first_task_action = templates_menu.addAction("Add PieceGenerator Template")
        ...
        for label, cls_name in [
            ...
            ("Non-Flexible Shutdowns", "simulation.flow.NonFlexibleShutdownsNode"),
            ("Flexible Shutdowns", "simulation.flow.FlexibleShutdownsNode"),
    ...
    def on_node_double_clicked(self, node):
        ...
        elif kind in {"PieceGenerator", "FirstTask"}:
            current = get_property_json(node, "models_goals", [])
            if not current and node.has_property("models_probs"):
                # Legacy conversion: probabilities are not enough for the new generator, use a default goal of 1.
                current = [{"model": item.get("model"), "goal": 1} for item in get_property_json(node, "models_probs", [])]
            dlg = PieceGeneratorGoalsDialog(self, self.model_registry, current)
            if dlg.exec():
                set_property_json(node, "models_goals", dlg.values())
    ...
    def validate_graph(self) -> List[str]:
        ...
        for node in self.all_nodes():
            ...
            if kind == "Task":
                ...
                if not get_input_ref(node, "task_duration") and not get_property_json(node, "models_configs", []):
                    problems.append(f"Task '{node.name()}' has no task_duration distribution and no advanced per-model durations.")
                if not get_input_ref(node, "startup_duration"):
                    problems.append(f"Task '{node.name()}' has no startup_duration distribution.")
                if not get_property_json(node, "capability", []) and not get_property_json(node, "models_configs", []):
                    problems.append(f"Task '{node.name()}' has no capability/models_configs.")

                min_carriers = get_int_prop_with_aliases(node, ("min_carriers",), 1)
                max_capacity = as_float(node.get_property("max_capacity"), 1.0)
                min_carrier_capacity = as_float(node.get_property("min_carrier_capacity"), 1.0)
                max_carrier_capacity = as_float(node.get_property("max_carrier_capacity"), 1.0)

                if min_carrier_capacity > max_carrier_capacity:
                    problems.append(f"Task '{node.name()}' has min_carrier_capacity > max_carrier_capacity.")
                if max_carrier_capacity > max_capacity:
                    problems.append(f"Task '{node.name()}' has max_carrier_capacity > max_capacity.")
                if min_carriers * min_carrier_capacity > max_capacity:
                    problems.append(f"Task '{node.name()}' has min_carriers × min_carrier_capacity > max_capacity.")

            elif kind in {"PieceGenerator", "FirstTask"}:
                goals = get_property_json(node, "models_goals", [])
                if not goals:
                    problems.append(f"PieceGenerator '{node.name()}' has no model goals.")
                for item in goals:
                    if as_int(item.get("goal", 0), 0) <= 0:
                        problems.append(f"PieceGenerator '{node.name()}' has a non-positive goal.")
                if as_float(node.get_property("working_hours") if node.has_property("working_hours") else 0, 0) <= 0:
                    problems.append(f"PieceGenerator '{node.name()}' has non-positive working_hours.")
            ...
            elif kind in {"NonFlexibleShutdowns", "FlexibleShutdowns", "ScheduledShutdowns"}:
    ...
    def ensure_template_model_names(self) -> List[str]:
        ...
        Only creates DummyModel if there is no model at all.
        This prevents templates from polluting an existing model registry.
        """
    ...
    def default_model_probabilities(self) -> List[dict]:
        names = self.ensure_template_model_names()
    ...
    def set_task_default_capability(self, task_node):
        self.set_json_property_safe(
            task_node,
            "capability",
            self.ensure_template_model_names(),
        )

    def set_hard_buffer_default_models(self, buffer_node):
        self.set_json_property_safe(
            buffer_node,
            "valid_models",
            self.ensure_template_model_names(),
    ...
    def create_buffer_template(self, name: str, x: float, y: float, model_name=None):
        node = self.make_node("simulation.flow.HardBufferNode", name, x, y)

        self.set_hard_buffer_default_models(node)
        self.set_property_safe(node, "capacity", "inf")

        return node

    def create_shutdowns_template(self, name: str, x: float, y: float):
        node = self.make_node("simulation.flow.NonFlexibleShutdownsNode", name, x, y)
        return node
    ...
    def add_task_template(self):
        ...
        # Defaults.
        self.set_task_default_capability(task)
        ...
        self.set_property_safe(task, "collector_type", "DISCRIMINATING_GREEDY")
        ...
        connect_ports_by_name(shutdowns, "non_flexible_shutdowns", task, "non_flexible_shutdowns")
    ...
    def add_first_task_template(self):
        """Creates a clean left-to-right PieceGenerator template without output buffers."""
        existing_bounds = self.content_bounds()
        generator = self.make_node("simulation.flow.PieceGeneratorNode", "PieceGenerator Template", -120, 180)
        default_models = self.ensure_template_model_names()
        goals = [{"model": model, "goal": 100} for model in default_models]
        self.set_json_property_safe(generator, "models_goals", goals)
        set_text_prop(generator, "working_hours", 480.0)
        group_nodes = [generator]
        self.place_clear_of_existing(group_nodes, existing_bounds)
        backdrop = self.add_backdrop_for_nodes(group_nodes, "PieceGenerator Template")
        self.focus_on_nodes(group_nodes + ([backdrop] if backdrop else []))
        self.statusBar().showMessage("Added clean left-to-right PieceGenerator template.")

    def node_type_from_kind(self, kind: str) -> str:
        mapping = {
            "Distribution": "simulation.flow.DistributionNode",
            "Interval": "simulation.flow.IntervalNode",
            "ScheduledShutdowns": "simulation.flow.NonFlexibleShutdownsNode",
            "NonFlexibleShutdowns": "simulation.flow.NonFlexibleShutdownsNode",
            "FlexibleShutdowns": "simulation.flow.FlexibleShutdownsNode",
            "Resource": "simulation.flow.ResourceNode",
            ...
            "FirstTask": "simulation.flow.PieceGeneratorNode",
            "PieceGenerator": "simulation.flow.PieceGeneratorNode",
            "Task": "simulation.flow.TaskNode",
            "PieceTask": "simulation.flow.TaskNode",
            "Breakdown": "simulation.flow.BreakdownNode",
            "Monitor": "simulation.flow.MonitorNode",
        }
    ...
    def apply_clean_json_to_node(self, node, node_data: dict):
        ...
        if kind == "Distribution":
        ...
        elif kind in {"FirstTask", "PieceGenerator"}:
            if "models_goals" in node_data:
                self.set_json_property_safe(node, "models_goals", node_data.get("models_goals", []))
            else:
                # Legacy conversion from probability-based source: keep models and give each a default goal of 1.
                goals = [{"model": item.get("model"), "goal": 1} for item in node_data.get("models_probs", []) if item.get("model")]
                self.set_json_property_safe(node, "models_goals", goals)
        ...
        elif kind in {"Task", "PieceTask"}:
            self.set_json_property_safe(node, "capability", node_data.get("capability", []))
            self.set_json_property_safe(node, "models_configs", node_data.get("models_configs", []))
            ...
            self.set_property_safe(node, "max_capacity", node_data.get("max_capacity", node_data.get("max_carrier_capacity", 1)))
            self.set_property_safe(node, "min_carrier_capacity", node_data.get("min_carrier_capacity", node_data.get("min_capacity", 1)))
            self.set_property_safe(node, "max_carrier_capacity", node_data.get("max_carrier_capacity", node_data.get("max_capacity", 1)))
            self.set_property_safe(node, "contiguous_carriers", node_data.get("contiguous_carriers", node_data.get("greedy_carriers", False)))
            self.set_property_safe(
                node,
                "collector_type",
                normalize_batch_collector(node_data.get("collector_type", node_data.get("batch_collector_type", node_data.get("batch_collector", "GREEDY")))),
            )
            self.set_property_safe(node, "independent_carriers", node_data.get("independent_carriers", False))

            resource_quantities = {}
            for item in flatten_resource_items(node_data.get("resources", [])):
                rid = item.get("resource")
            ...
            operator_quantities = {}
            for item in flatten_resource_items(node_data.get("operators", [])):
                rid = item.get("resource")
            ...
            for item in flatten_resource_items(node_data.get("startup_operators", [])):
                rid = item.get("resource")
                quantity = item.get("quantity", 1)
            ...
            self.set_json_property_safe(node, "default_resource_groups", node_data.get("default_resource_groups", normalize_groups_from_flat(node_data.get("resources", []), quantity_type=float)))
            self.set_json_property_safe(node, "operator_groups", node_data.get("operators", normalize_groups_from_flat(node_data.get("operators", []), quantity_type=int)))
            self.set_json_property_safe(node, "startup_operator_groups", node_data.get("startup_operators", normalize_groups_from_flat(node_data.get("startup_operators", []), quantity_type=int)))
