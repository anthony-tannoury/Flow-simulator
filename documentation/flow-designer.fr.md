...
- **Déplacement :** glisser le fond du canevas.
- **Zoom :** molette de la souris.
- **Sélection :** cliquer une carte ; tracer un rectangle ou utiliser le clic avec modificateur pour une sélection multiple.
...
Un panneau **Properties** affichant les champs bruts d'une carte est disponible depuis le menu Tools. Il est masqué par défaut ; la configuration des cartes s'effectue normalement par les boîtes de réglages décrites en section 6.
...
|---|---|
| Piece generator | La source de pièces. Exactement une par modèle. |
| Buffer | Une file : passage, sortie, ou rebut. |
...
- Générateur vers buffer : où arrivent les nouvelles pièces.
...
Pour retirer un fil, le sélectionner et le supprimer.
...
- **Translation :** créer un nouveau shift comme copie décalée dans le temps d'un shift existant, pour des équipes en quinconce sur un même motif.
...
- **Type de collecteur** et règle de modèle focus.
- **Timeout, priorité, drapeau admin.**
- **Politiques :** les sélections de protocoles (contraintes de shift, traitement des carriers en attente avant un arrêt, conscience de soi des opérateurs, ordre de sortie des pièces). Les valeurs par défaut conviennent à la plupart des stations.
- **Shifts du poste :** le planning d'exploitation de la station.
...
- **Graine (seed) :** la graine aléatoire. Une graine et un modèle donnés reproduisent exactement la même exécution.
...
Les deux moteurs produisent les mêmes fichiers de sortie avec la même structure. Le choix du moteur n'affecte que la vitesse d'exécution.
...
- Le canevas est verrouillé contre l'édition.
- Le double-clic sur une carte ouvre ses indicateurs : production et attentes pour un poste, statistiques de file pour un buffer, occupation pour un groupe d'opérateurs.
...
Tous les fichiers CSV s'ouvrent directement dans Excel. La description complète de chaque fichier et de chaque indicateur, conventions de mesure et guide d'interprétation compris, se trouve dans la **[référence des KPI](kpis.fr.md)**.

---

## 14. Construire un premier modèle

Le flux de travail minimal :

1. **Registres :** un modèle, un groupe d'opérateurs, un shift.
2. **Cartes :** un générateur de pièces, un buffer d'entrée, un poste à pièces, un buffer de sortie.
3. **Câblage :** générateur vers buffer, buffer vers poste, poste vers buffer de sortie.
4. **Configuration :** types de buffers et modèles valides ; durée de traitement du poste, opérateurs, et shift.
5. **Réglages de simulation :** mode objectifs, un objectif pour le modèle, une date de début.
6. **Valider**, résoudre les signalements.
7. **Exécuter**, puis **View results**.

Les extensions suivent le même schéma : créer une carte, la câbler, la configurer. Les étapes suivantes typiques sont un routeur avec un buffer de rebut pour le tri qualité, des stations supplémentaires, et des pannes ou arrêts sur les équipements critiques.

---

## Documents associés

- Concepts et sémantique des réglages : [référence de la simulation](simulation.fr.md).
- Interprétation des sorties : [référence des KPI](kpis.fr.md).
