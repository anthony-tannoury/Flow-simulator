...
BATCH_COLLECTORS = ["GREEDY", "ALTRUISTIC"]
...
def normalize_batch_collector(value: Any) -> str:
    """Normalize old UI names to the final simulator enum names."""
    mapping = {
        "GreedyBatchCollector": "GREEDY",
        "AltruisticBatchCollector": "ALTRUISTIC",
        "GREEDY": "GREEDY",
        "ALTRUISTIC": "ALTRUISTIC",
    }
    return mapping.get(str(value), "GREEDY")
...
class FirstTaskNode(SimNode):
    ...
    def to_clean_json(self) -> dict:
        ...
        models_probs = []
        for item in get_property_json(self, "models_probs", []):
            models_probs.append({
                "model": item.get("model"),
                "probability": as_float(item.get("probability", 0.0), 0.0),
                "batch_size": as_int(item.get("batch_size", item.get("batch", 1)), 1),
            })
        return {
            "id": node_uid(self),
            "kind": self.kind,
            "name": self.name(),
            "models_probs": models_probs,
            "resources": [
...
class TaskNode(SimNode):
    ...
    def __init__(self):
        ...
        # Final simulator TaskConfig fields.
        # Legacy min_capacity/max_capacity clean JSON is still accepted on import.
        ...
        self.create_property("batch_collector_type", "GREEDY")
    ...
    def to_clean_json(self) -> dict:
        ...
        return {
            ...
            "max_capacity": get_int_prop_with_aliases(self, ("max_capacity",), 1),
            "min_carrier_capacity": get_int_prop_with_aliases(self, ("min_carrier_capacity", "min_capacity"), 1),
            "max_carrier_capacity": get_int_prop_with_aliases(self, ("max_carrier_capacity", "max_capacity"), 1),
            "contiguous_carriers": get_bool_prop_with_aliases(self, ("contiguous_carriers", "greedy_carriers"), False),
            "batch_collector_type": normalize_batch_collector(
                self.get_property("batch_collector_type")
                if self.has_property("batch_collector_type")
                else self.get_property("batch_collector")
            ),
...
class WeightedModelsDialog(QtWidgets.QDialog):
    def __init__(self, parent, all_models: List[dict], current: List[dict]):
        super().__init__(parent)
        self.setWindowTitle("FirstTask model probabilities and batch sizes")
        self.resize(620, 400)

        self.all_model_names = [m["name"] for m in all_models]

        self.table = QtWidgets.QTableWidget(0, 3)
        self.table.setHorizontalHeaderLabels(["model", "probability", "batch_size"])
        self.table.horizontalHeader().setStretchLastSection(True)
        ...
        layout = QtWidgets.QVBoxLayout(self)
        layout.addWidget(QtWidgets.QLabel("Pick models, probabilities, and generated batch sizes. Probabilities must sum to 1."))
        layout.addWidget(self.table)
        layout.addLayout(tools)
        layout.addLayout(bottom)

        btn_add.clicked.connect(lambda: self.add_row("", 0.0, 1))
        btn_remove.clicked.connect(self.remove_selected)
        ...
        for item in current:
            self.add_row(
                ...
                item.get("batch_size", item.get("batch", 1)),
            )

    def add_row(self, model="", probability=0.0, batch_size=1):
        row = self.table.rowCount()
        self.table.insertRow(row)
        ...
        self.table.setItem(row, 2, QtWidgets.QTableWidgetItem(str(batch_size)))
    ...
    def values(self) -> List[dict]:
        ...
        for r in range(self.table.rowCount()):
            ...
            prob_item = self.table.item(r, 1)
            batch_item = self.table.item(r, 2)
            model = combo.currentText().strip() if combo else ""
            prob = as_float(prob_item.text() if prob_item else 0.0, 0.0)
            batch_size = as_int(batch_item.text() if batch_item else 1, 1)
            ...
            seen.add(model)
            total += prob
            result.append({"model": model, "probability": prob, "batch_size": batch_size})
    ...
    def accept(self):
        try:
            ...
            qmessage(self, "Invalid probabilities or batch sizes", str(e), QtWidgets.QMessageBox.Warning)
            return
        super().accept()
...
class TaskConfigDialog(QtWidgets.QDialog):
    def __init__(self, parent, task_node: TaskNode, model_registry: List[dict]):
        ...
        self.max_capacity.setValue(get_int_prop_with_aliases(task_node, ("max_capacity",), 1))

        self.min_carrier_capacity = QtWidgets.QSpinBox()
        self.min_carrier_capacity.setMinimum(1)
        self.min_carrier_capacity.setMaximum(999999)
        self.min_carrier_capacity.setValue(
            get_int_prop_with_aliases(task_node, ("min_carrier_capacity", "min_capacity"), 1)
        )

        self.max_carrier_capacity = QtWidgets.QSpinBox()
        self.max_carrier_capacity.setMinimum(1)
        self.max_carrier_capacity.setMaximum(999999)
        self.max_carrier_capacity.setValue(
            get_int_prop_with_aliases(task_node, ("max_carrier_capacity", "max_capacity"), 1)
        )

        self.contiguous_carriers = QtWidgets.QCheckBox("contiguous_carriers")
        self.contiguous_carriers.setChecked(
            get_bool_prop_with_aliases(task_node, ("contiguous_carriers", "greedy_carriers"), False)
        )

        self.batch_collector_type = QtWidgets.QComboBox()
        self.batch_collector_type.addItems(BATCH_COLLECTORS)
        current_batch_collector = (
            task_node.get_property("batch_collector_type")
            if task_node.has_property("batch_collector_type")
            else task_node.get_property("batch_collector")
        )
        self.batch_collector_type.setCurrentText(normalize_batch_collector(current_batch_collector))
        ...
        form.addRow("batch_collector_type", self.batch_collector_type)
    ...
    def accept(self):
        if self.min_carrier_capacity.value() > self.max_carrier_capacity.value():
            qmessage(
                self,
                "Invalid carrier capacity",
                "min_carrier_capacity cannot be greater than max_carrier_capacity.",
        ...
        if self.max_carrier_capacity.value() > self.max_capacity.value():
            qmessage(
                self,
                "Invalid task capacity",
                "max_carrier_capacity cannot be greater than max_capacity.",
                QtWidgets.QMessageBox.Warning,
            )
            return
        ...
        if self.min_carriers.value() * self.min_carrier_capacity.value() > self.max_capacity.value():
            qmessage(
                self,
                "Invalid task capacity",
                "min_carriers × min_carrier_capacity cannot be greater than max_capacity.",
                QtWidgets.QMessageBox.Warning,
            )
            return
        ...
        self.task_node.set_property("batch_collector_type", self.batch_collector_type.currentText())
...
class FlowEditorWindow(QtWidgets.QMainWindow):
    ...
    def validate_graph(self) -> List[str]:
        ...
        for node in self.all_nodes():
            ...
            if kind == "Task":
                ...
                max_capacity = get_int_prop_with_aliases(node, ("max_capacity",), 1)
                min_carrier_capacity = get_int_prop_with_aliases(node, ("min_carrier_capacity", "min_capacity"), 1)
                max_carrier_capacity = get_int_prop_with_aliases(node, ("max_carrier_capacity", "max_capacity"), 1)
                ...
                for item in models_probs:
                    if as_int(item.get("batch_size", item.get("batch", 1)), 1) <= 0:
                        problems.append(f"FirstTask '{node.name()}' has a non-positive batch_size.")
                if not get_input_ref(node, "task_duration"):
    ...
    def add_task_template(self):
        ...
        self.set_property_safe(task, "batch_collector_type", "GREEDY")
    ...
    def add_first_task_template(self):
        ...
        self.set_json_property_safe(
            first_task,
            "models_probs",
            self.default_model_probabilities(),
        )
    ...
    def apply_clean_json_to_node(self, node, node_data: dict):
        ...
        if kind == "Distribution":
            ...
            self.set_property_safe(
                node,
                "batch_collector_type",
                normalize_batch_collector(node_data.get("batch_collector_type", node_data.get("batch_collector", "GREEDY"))),
            )
