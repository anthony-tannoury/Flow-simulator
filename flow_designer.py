"""
Simulation Flow Designer (NodeGraphQt) — matched to the new simulation.py.

Goal
----
Build a typed node graph and export a clean JSON file that a parser can turn
into the simulation.py classes. It is NOT a simulation runner.

Install
-------
pip install NodeGraphQt PySide6 Qt.py
# or PySide2, depending on your environment.

Run
---
python flow_designer.py

What changed vs the old designer (to match the new simulation)
--------------------------------------------------------------
- Source (PieceGenerator): the generator now takes {model: integer goal} and
  working_hours (it derives the inter-arrival gap itself). So the Source card no
  longer has a duration input or per-model probabilities; instead it has a
  "working_hours" field and a model -> integer goal picker.
- Piece Task: config is now PER MODEL. Each model the task can handle has its own
  duration distribution and its own resource groups (models_configs). Edit them
  via "Edit model configs...". collector_type now exposes the four CollectorType
  values (discriminating x greedy/altruistic).
- Shutdowns: the sim has NonFlexibleShutdowns and FlexibleShutdowns. One
  "Shutdowns" card carries a type toggle and feeds a task's multi-input
  "shutdowns" port.
- Resource Task: a new card for ResourceTask (non-transformed resources,
  transformed/salvageable resources, produced output resources, task_duration,
  resource_collector_type, ...). NOTE: the resource-task side of the sim is
  experimental ("reste a faire").
- Auto-layout was removed.
- Pipes are drawn as right-angle (horizontal/vertical) elbows for a cleaner canvas.

Notes
-----
- Models are global, edited from "Models > Edit models...".
- Only clean JSON import/export is supported. Import is ADDITIVE (fresh ids,
  shifted clear of existing content) so subflows compose. Use "File > New" to
  start blank.
- SoftBuffer probabilities are stored by connected target node id.
- Backdrops round-trip their wrapped node ids and their size.
"""
...
class SoftBufferNode(SimNode):
    ...
    def to_clean_json(self) -> dict:
        ...
        for buffer_id in connected_buffers:
            buffer_probs.append({"buffer": buffer_id, "probability": as_float(prob_map.get(buffer_id, 0.0), 0.0)})
        return {
            "id": node_uid(self),
            "kind": self.kind,
            "name": self.name(),
...
class FirstTaskNode(SimNode):
    """PieceGenerator: {model: integer goal} over working_hours -> outlets."""
...
class TaskNode(SimNode):
    """PieceTask. Per-model duration + resources (models_configs)."""
    ...
    def __init__(self):
        ...
        self.add_input("durations", multi_input=True, color=PORT_COLORS["duration"])
        ...
        # models_configs = [{model, duration(dist id), resources:{resource_id: qty}}]
        self.create_property("models_configs", "[]")
        self.create_property("resources_scope", "PER_BATCH")   # PER_PIECE | PER_BATCH
        self.create_property("operators_scope", "PER_BATCH")   # PER_BATCH | PER_TASK
    ...
    def to_clean_json(self) -> dict:
        model_configs = get_property_json(self, "models_configs", [])
        capability = [mc.get("model") for mc in model_configs if mc.get("model")]
        models_configs = []
        for mc in model_configs:
            res = mc.get("resources") or {}
            models_configs.append({
                "model": mc.get("model"),
                "duration": mc.get("duration"),
                "resources": [{"resource": rid, "quantity": as_float(q, 1.0)} for rid, q in res.items()],
            })
        ...
        return {
            ...
            "operators": [{"resource": rid, "quantity": as_int(oq.get(rid, 1), 1)} for rid in operators],
            "operators_scope": self.get_property("operators_scope"),
            "resources_scope": self.get_property("resources_scope"),
            "startup_operators": [{"resource": rid, "quantity": as_int(soq.get(rid, 1), 1)} for rid in startup_ops],
            ...
            "min_carrier_capacity": as_int(self.get_property("min_carrier_capacity"), 1),
            "max_carrier_capacity": as_int(self.get_property("max_carrier_capacity"), 1),
...
class ResourceTaskNode(SimNode):
    """ResourceTask (experimental in the sim). Transforms quantities of resources."""
    ...
    def __init__(self):
        super().__init__()
        self.add_input("non_transformed_resources", multi_input=True, color=PORT_COLORS["resource"])
        self.add_input("transformed_resources", multi_input=True, color=PORT_COLORS["resource"])
        self.add_input("resources_out", multi_input=True, color=PORT_COLORS["resource"])
        self.add_input("operators", multi_input=True, color=PORT_COLORS["resource"])
        self.add_input("startup_operators", multi_input=True, color=PORT_COLORS["resource"])
        ...
        self.create_property("resources_scope", "PER_BATCH")
        ...
        self.create_property("contiguous_carriers", False)
        self.create_property("independent_carriers", False)
        self.create_property("non_transformed_quantities", "{}")   # {rid: qty}
        self.create_property("operator_quantities", "{}")
        self.create_property("startup_operator_quantities", "{}")
        self.create_property("transformed_specs", "{}")            # {rid: {proportion, salvageable}}
        self.create_property("out_specs", "{}")                    # {rid: {dist_type, params, low, high}}

    def to_clean_json(self) -> dict:
        non_transformed = connected_refs_from_port(self, "non_transformed_resources", "input")
        transformed = connected_refs_from_port(self, "transformed_resources", "input")
        out_resources = connected_refs_from_port(self, "resources_out", "input")
        operators = connected_refs_from_port(self, "operators", "input")
        startup_ops = connected_refs_from_port(self, "startup_operators", "input")

        ntq = get_property_json(self, "non_transformed_quantities", {})
        oq = get_property_json(self, "operator_quantities", {})
        soq = get_property_json(self, "startup_operator_quantities", {})
        tspecs = get_property_json(self, "transformed_specs", {})
        ospecs = get_property_json(self, "out_specs", {})

        transformed_out = []
        for rid in transformed:
            spec = tspecs.get(rid, {})
            transformed_out.append({
                "resource": rid,
                "proportion": as_float(spec.get("proportion", 1.0), 1.0),
                "salvageable": bool(spec.get("salvageable", True)),
            })

        resources_out_distr = []
        for rid in out_resources:
            spec = ospecs.get(rid, {})
            resources_out_distr.append({
                "resource": rid,
                "distribution": {
                    "type": spec.get("dist_type", "Normal"),
                    "params": spec.get("params", {"mean": 1.0, "std": 0.0}),
                },
                "low": as_float(spec.get("low", 0.0), 0.0),
                "high": as_float(spec.get("high", 1.0), 1.0),
            })
        ...
        return {
            ...
            "kind": self.kind,
            "name": self.name(),
            "non_transformed_resources": [{"resource": rid, "quantity": as_float(ntq.get(rid, 1.0), 1.0)} for rid in non_transformed],
            "transformed_resources_salvageable": transformed_out,
            "resources_out_distr": resources_out_distr,
            "operators": [{"resource": rid, "quantity": as_int(oq.get(rid, 1), 1)} for rid in operators],
            "startup_operators": [{"resource": rid, "quantity": as_int(soq.get(rid, 1), 1)} for rid in startup_ops],
            "operators_scope": self.get_property("operators_scope"),
            "resources_scope": self.get_property("resources_scope"),
...
class BreakdownNode(SimNode):
    ...
    def __init__(self):
        ...
        self.add_output("bufs_out", multi_output=True, color=PORT_COLORS["task"])
        add_float_input(self, "A", "A (early-life)", 0.0)
        add_float_input(self, "tau", "tau", 1.0)
        add_float_input(self, "c", "c (constant rate)", 0.01)
        add_float_input(self, "beta", "beta (wear shape)", 1.0)
        add_float_input(self, "eta", "eta (wear scale)", 1.0)
    ...
    def to_clean_json(self) -> dict:
        return {
            ...
            "failure_rate": {k: as_float(self.get_property(k), d) for k, d in
                             (("A", 0.0), ("tau", 1.0), ("c", 0.01), ("beta", 1.0), ("eta", 1.0))},
...
class ModelConfigsDialog(QtWidgets.QDialog):
    """Per-model config for a Piece Task: model -> (duration card, resource quantities)."""
    def __init__(self, parent, model_registry, duration_nodes, resource_nodes, current):
        super().__init__(parent)
        self.setWindowTitle("Model configs (duration + resources per model)")
        self.resize(640, 420)
        ...
        # row index -> {resource_id: qty}
        self.row_resources = {}

        self.table = QtWidgets.QTableWidget(0, 3)
        self.table.setHorizontalHeaderLabels(["model", "duration (connected)", "resources"])
        self.table.horizontalHeader().setStretchLastSection(True)
        ...
        layout.addWidget(QtWidgets.QLabel(
            ...
            "then assign one duration and resource quantities to each model the task handles."))
        layout.addWidget(self.table)
        layout.addLayout(tools)
        layout.addLayout(bottom)
        ...
        for mc in current:
            self.add_row(mc.get("model", ""), mc.get("duration"), dict(mc.get("resources") or {}))
    ...
    def add_row(self, model="", duration_id=None, resources=None):
        row = self.table.rowCount()
        self.table.insertRow(row)
        self.row_resources[row] = dict(resources or {})
        ...
        btn = QtWidgets.QPushButton("Edit resources...")
        btn.clicked.connect(lambda _=False, r=row: self.edit_resources(r))
        self.table.setCellWidget(row, 2, btn)

    def edit_resources(self, row):
        current = self.row_resources.get(row, {})
        dlg = QuantityDialog(self, "Resource quantities for this model", self.resource_nodes, current, integer=False)
        if dlg.exec():
            self.row_resources[row] = dlg.values()

    def remove_selected(self):
        rows = sorted({i.row() for i in self.table.selectedIndexes()}, reverse=True)
        for r in rows:
            self.table.removeRow(r)
        # Rebuild row_resources contiguously.
        self._reindex()

    def _reindex(self):
        new_map = {}
        for r in range(self.table.rowCount()):
            new_map[r] = self.row_resources.get(r, {})
        self.row_resources = new_map
    ...
    def values(self) -> List[dict]:
        ...
        for r in range(self.table.rowCount()):
            ...
            if model in seen:
                raise ValueError(f"Duplicate model: {model}")
            seen.add(model)
            result.append({"model": model, "duration": duration_id, "resources": self.row_resources.get(r, {})})
        if not result:
...
class FlowEditorWindow(QtWidgets.QMainWindow):
    ...
    def _build_menus(self):
        ...
        templates_menu.addAction("Add Piece Task Template").triggered.connect(self.add_task_template)
        templates_menu.addAction("Add Source Template").triggered.connect(self.add_first_task_template)
        ...
        for label, cls_name in [
            ...
            ("Soft Buffer (Router)", "simulation.flow.SoftBufferNode"),
    ...
    def on_node_double_clicked(self, node):
        ...
        if kind == "Distribution":
            ...
            dlg = ModelGoalsDialog(self, self.model_registry, current)
            if dlg.exec():
            ...
            TaskConfigDialog(self, node, self.model_registry).exec()

        elif kind == "ResourceTask":
            ResourceTaskConfigDialog(self, node).exec()

        elif kind in {"Resource", "RestockableResource"}:
            ResourceConfigDialog(self, node).exec()

        elif kind == "Breakdown":
            qmessage(self, "Breakdown card",
                     "Connect:\n- Task/ResourceTask.task_ref -> Breakdown.task\n"
                     "- Distribution -> Breakdown.mttr\n"
                     "- Breakdown.bufs_out -> HardBuffer/SoftBuffer (emergency)\n\n"
                     "Set the bathtub failure-rate params (A, tau, c, beta, eta) on the card.")
    ...
    def add_task_template(self):
        ...
        self.set_json_property_safe(task, "models_configs", [{
            ...
            "resources": {node_uid(restockable): 1.0},
        }])
    ...
    def apply_clean_json_to_node(self, node, node_data: dict):
        ...
        if kind == "Distribution":
            ...
            for mc in node_data.get("models_configs", []):
                res = {}
                for it in mc.get("resources", []):
                    if it.get("resource"):
                        res[it["resource"]] = it.get("quantity", 1.0)
                models_configs.append({"model": mc.get("model"), "duration": mc.get("duration"), "resources": res})
            ...
            oq = {it["resource"]: it.get("quantity", 1) for it in node_data.get("operators", []) if it.get("resource")}
            sq = {it["resource"]: it.get("quantity", 1) for it in node_data.get("startup_operators", []) if it.get("resource")}
            self.set_json_property_safe(node, "operator_quantities", oq)
            self.set_json_property_safe(node, "startup_operator_quantities", sq)
