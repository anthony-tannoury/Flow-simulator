...
------------------------
Ce programme imite le fonctionnement d'un atelier de production :
...
(une pièce arrive, une machine finit, une panne survient...). L'unité de temps
est libre : si vous donnez des durées en minutes, alors tout est en minutes.
...
--------------
  ...
  - CHARIOT (Carrier)     : voir l'explication détaillée plus bas.
...
# la reproductibilité des résultats et pour le débogage.
# Changez la graine pour obtenir un autre tirage du hasard.
SEED = 42
sim.yieldless(True)
...
def check_probs(probs: list[float]) -> None:
    ...
    et leur somme égale à 1. Sert pour les aiguillages et la génération
...
class Model:
    ...
    def __init__(self, name: str, parent: Model | None):
        self.name = name
        self.parent = parent
        self.children = []
...
class RestockableResource(sim.Resource):
    ...
    Le réapprovisionnement se fait en deux temps :
...
class Piece(sim.Component):
    ...
    Paramètre :
      - model : le type de la pièce. DOIT être un modèle feuille (sans sous-types).
    """
...
class Buffer(sim.Store, Outlet):
    """
    UNE ZONE DE STOCKAGE / FILE D'ATTENTE entre deux postes.
...
class Breakdown(sim.Component):
    ...
    Le poste accumule un risque au fil du temps ; quand ce risque dépasse un seuil
    tiré au sort, la panne survient. Le taux de panne peut varier dans le temps
    (failure_rate est une fonction du temps), ce qui permet de représenter la courbe
    en baignoire : beaucoup de pannes au rodage, peu en régime stable, de plus en
    plus en fin de vie.
    ...
    Paramètres :
      ...
      - failure_rate  : fonction donnant le taux de panne à un instant t. Plus il
                        est élevé, plus les pannes sont fréquentes.
    ...
    def process(self):
        while True:
            # On ne compte pas de pannes pendant un arrêt planifié : une machine
            # éteinte ne tombe pas en panne. On attend donc qu'elle tourne.
...
class TaskStarter(sim.Component):
    ...
    Si un arrêt planifié est imminent et que la mise en route n'aurait pas le
    temps de finir avant, on attend la fin de l'arrêt pour démarrer (inutile
    de lancer un démarrage qui serait aussitôt coupé). Si l'on n'arrive pas à
    mobiliser les opérateurs de démarrage à temps, le poste reste gelé jusqu'au
    prochain redémarrage possible.
    """
...
class Carrier(sim.Component, ABC):
    ...
    Selon le réglage du poste, plusieurs chariots peuvent exister en même temps
    (plusieurs fournées en parallèle) en se partageant la capacité totale de la machine.
...
class Task(sim.Component, ABC):
    """
    UN POSTE DE TRAVAIL (ou machine). C'est lui qui, en boucle, démarre, prépare
    un lot (via un chariot), le lance, puis recommence.
    ...
    Trois états résument sa situation :
      - is_in_breakdown : le poste est en panne.
