...
gen_shifts = [Interval(0, 480000), Interval(600000, 1080000)]
b0 = Buffer("B0", valid_models=[model_a, model_b], buffer_type=BufferType.PASSAGE)
gen = PieceGenerator(models_goals={model_a: 12000, model_b: 8000},
                     shifts=gen_shifts, outlets=[b0])

og1 = OperatorGroup("og1", capacity=2,
                    shifts=[Interval(0, 1500000)],
...
t1_config = PieceTaskConfig(
    task_shifts=[Interval(0, 1500000)],
...
criterion = ByTime(time=1500000)
SimulationStopper(criterion=criterion)

env.run(till=100000000)
